...
define( 'BASEL_POST_TYPE_VERSION', '1.16' );
...
function customize_woocommerce_product_tabs( $tabs ) {
    // Check if 'additional_information' tab exists
    if ( isset( $tabs['additional_information'] ) ) {
        // Change the title to "DESCRIPTION"
        $tabs['additional_information']['title'] = __( 'Description', 'your-textdomain' );
    }
    return $tabs;
}
add_filter( 'woocommerce_product_tabs', 'customize_woocommerce_product_tabs' );
...
function add_custom_product_tabs( $tabs ) {
    // Add ORDER PROCESS tab
    $tabs['order_process'] = array(
        'title'    => __( 'Order Process', 'your-textdomain' ), // Tab title
        'priority' => 30, // Adjust the order (lower numbers appear first)
        'callback' => 'order_process_tab_content', // Content callback function
    );

    // Add SHIPPING & RETURNS tab
    $tabs['shipping_returns'] = array(
        'title'    => __( 'Shipping & Returns', 'your-textdomain' ), // Tab title
        'priority' => 40, // Adjust the order
        'callback' => 'shipping_returns_tab_content', // Content callback function
    );
...
function order_process_tab_content() {
    echo '<h2>Our Order Process</h2>';
    echo '<img src="https://realleathergarments.co.uk/wp-content/uploads/2024/11/ORDER-QUE-01.png">';
}

function shipping_returns_tab_content() {
    echo '<h2>Shipping & Returns</h2>';
    echo '<p>Provide details about your shipping policies, estimated delivery times, and return/exchange process here.</p>';
}


// add to cart page to redirect checkout page

add_filter( 'woocommerce_cart_needs_payment', 'ibrar_disable_payment_on_cart_page' );
function ibrar_disable_payment_on_cart_page( $needs_payment ) {
	if ( is_cart() ) {
		return false;
	}
...
// add_filter('woocommerce_add_to_cart_redirect', 'rlg_redirect_to_checkout');
// function rlg_redirect_to_checkout($url) {
//     return 'https://realleathergarments.co.uk/checkout/';
// }
